...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		HRESULT hr = sceneContext.d3dContext.pDevice->CreateBuffer(&vertexBufferDesc, nullptr, &m_pVertexBuffer);
		HANDLE_ERROR(hr);
	...
	if (m_pVertexBuffer)
	{
		...
		memcpy(mappedResource.pData, &m_Sprites[0], m_Sprites.size() * sizeof(VertexSprite));
