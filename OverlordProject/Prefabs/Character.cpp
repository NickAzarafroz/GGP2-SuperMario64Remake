...
void Character::Update(const SceneContext& sceneContext)
{
	if (m_pCameraComponent->IsActive())
	{
		...
		TransformComponent* pTransformComp{ GetTransform() };

		const XMVECTOR forwardVec{ XMLoadFloat3(&pTransformComp->GetForward()) };
		const XMVECTOR rightVec{ XMLoadFloat3(&pTransformComp->GetRight()) };
		...
		const float rotationAmount = m_CharacterDesc.rotationSpeed * sceneContext.pGameTime->GetElapsed();

		m_TotalYaw += look.x * rotationAmount;
		//m_TotalPitch += look.y * rotationAmount;

		GetTransform()->Rotate(0.f, m_TotalYaw, 0.0f);
		//sceneContext.pCamera->GetTransform()->Rotate(std::clamp(m_TotalPitch, -10.f, 10.f), 0.f, 0.f);
		...
		const float currMoveAcceleration = m_MoveAcceleration * sceneContext.pGameTime->GetElapsed();
			...
			sceneContext.pInput->IsActionTriggered(m_CharacterDesc.actionId_MoveBackward))
		{
			...
			directionVec = rightVec * move.x + forwardVec * move.y;
			XMStoreFloat3(&m_CurrentDirection, directionVec);
			...
			{
				m_MoveSpeed += currMoveAcceleration;
			}
