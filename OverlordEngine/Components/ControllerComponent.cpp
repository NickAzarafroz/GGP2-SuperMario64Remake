...
void ControllerComponent::Initialize(const SceneContext&)
{
	if(!m_IsInitialized)
	{
		...
		auto& position = this->GetGameObject()->GetTransform()->GetPosition();
		m_ControllerDesc.position = PxExtendedVec3{ position.x, position.y, position.z };
		m_ControllerDesc.userData = this;

		auto physicsProxy = this->GetGameObject()->GetScene()->GetPhysxProxy();
		auto controllerManager = physicsProxy->GetControllerManager();

		m_pController = controllerManager->createController(m_ControllerDesc);

		if(!m_pController)
		{
			Logger::LogError(L"Returned controller doesnt exist!");
			return;
		}
