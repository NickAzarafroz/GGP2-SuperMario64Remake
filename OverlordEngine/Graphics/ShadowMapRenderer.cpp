...
void ShadowMapRenderer::Initialize()
{
	...
	RENDERTARGET_DESC renderTarget;
	renderTarget.enableColorBuffer = false;
	renderTarget.enableDepthSRV = true;
	renderTarget.width = 1280;
	renderTarget.height = 720;

	m_pShadowRenderTarget = new RenderTarget(m_GameContext.d3dContext);
	m_pShadowRenderTarget->Create(renderTarget);
...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	ShadowGeneratorType SGT{};

	if(pMeshFilter->HasAnimations())
	{
		SGT = ShadowGeneratorType::Skinned;
	}
	else
	{
		SGT = ShadowGeneratorType::Static;
	}

	const auto& techContext = m_GeneratorTechniqueContexts[static_cast<int>(SGT)];
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	const auto& lightProjMatrix = XMMatrixOrthographicLH(viewWidth, viewHeight, nearZ, farZ);
	const auto& dirLight = sceneContext.pLights->GetDirectionalLight();
	
	const auto& eyePos = XMLoadFloat4(&dirLight.position);
	const auto& focusPos = eyePos + XMLoadFloat4(&dirLight.direction);

	const auto& lightViewMatrix = XMMatrixLookAtLH(eyePos, focusPos, XMVECTOR{ 0.0f, 1.0f, 0.0f });
	const auto& lightVP = lightViewMatrix * lightProjMatrix;
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	for (const auto& subMesh : pMeshFilter->GetMeshes())
	{
		...
		m_pShadowMapGenerator->SetTechnique(static_cast<int>(SGT));
