...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float2 texCoord, float4 col, float2x2 uvRotation)
{
    ...
    float4 posTransformedH = mul(posH, gWorldViewProj);
    geomData.Position = posTransformedH;
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	//topLeft
    topLeft = origin + float3(-size * 0.5f, -size * 0.5f, 0.f);
	
	//topRight
    topRight = origin + float3(-size * 0.5f, size * 0.5f, 0.f);
	
	//bottomLeft
    bottomLeft = origin + float3(size * 0.5f, -size * 0.5f, 0.f);
	
	//bottomRight
    bottomRight = origin + float3(size * 0.5f, size * 0.5f, 0.f);
    ...
    float3x3 viewInvRot = (float3x3) gViewInverse; // Get the rotational part of the ViewInverse matrix
	
    topLeft = mul(float3(size * -0.5f, size * 0.5f, 0.0f), (float3x3) gViewInverse);
    topLeft += origin;
    topRight = mul(float3(size * 0.5f, size * 0.5f, 0.0f), (float3x3) gViewInverse);
    topRight += origin;
    bottomLeft = mul(float3(size * -0.5f, size * -0.5f, 0.0f), (float3x3) gViewInverse);
    bottomLeft += origin;
    bottomRight = mul(float3(size * 0.5f, size * -0.5f, 0.0f), (float3x3) gViewInverse);
    bottomRight += origin;
